...
class _ParkingDetailScreenState extends State<ParkingDetailScreen> {
  ...
  // Tariff Management State (for input and simulation)
  ...
  void _initializeTariffState(TariffConfig config) {
    ...
    try {
      ...
      _nightStartTime = TimeOfDay(hour: int.parse(startParts[0]), minute: int.parse(startParts[1]));
      final endParts = config.nightEndTime.split(':');
      _nightEndTime = TimeOfDay(hour: int.parse(endParts[0]), minute: int.parse(endParts[1]));
    } catch (_) {
      _nightStartTime = const TimeOfDay(hour: 22, minute: 0);
      _nightEndTime = const TimeOfDay(hour: 6, minute: 0);
    }
    
    // Deserialize raw list of dynamic maps into FlexRule objects for local state
    _flexRules = (config.flexRulesRaw as List<dynamic>?)
        ?.map((r) => FlexRule.fromTariffConfig(r))
        .toList() ?? [];
  }
  
  TariffConfig _getCurrentTariffConfig() {
    // Collects current state data into a single TariffConfig object
    return TariffConfig(
      ...
      nightStartTime: '${_nightStartTime.hour.toString().padLeft(2, '0')}:${_nightStartTime.minute.toString().padLeft(2, '0')}', 
      nightEndTime: '${_nightEndTime.hour.toString().padLeft(2, '0')}:${_nightEndTime.minute.toString().padLeft(2, '0')}', 
      // Convert FlexRule objects back to serializable maps
  ...
  void _onTariffDataChanged() {
    // Called by input widgets to trigger simulation and revenue update
    setState(() {
      _simulateCostProjection();
      _calculateProjectedRevenue();
    });
  ...
  Future<void> _updateTariff() async {
    ...
    final updatedParking = Parking(
      ...
      ratePerHour: config.dayBaseRate, // Use day rate as general rate_per_hour fallback
      tariffConfig: config, 
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tariff and Rules updated successfully'), backgroundColor: Colors.green),
      );
    } catch (e) {
       ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to update tariff: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Future<void> _addSpot() async {
    try {
      ...
      // Manually update spots list and counts for fast feedback
      setState(() {
        spots.add(newSpot);
        parking = Parking(
            id: parking!.id, name: parking!.name, city: parking!.city, address: parking!.address, 
            totalSpots: parking!.totalSpots + 1, occupiedSpots: parking!.occupiedSpots, ratePerHour: parking!.ratePerHour,
            tariffConfig: parking!.tariffConfig, latitude: parking!.latitude, longitude: parking!.longitude
        );
        _calculateProjectedRevenue();
      });
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteSpot(int spotId) async {
    try {
      ...
      if (success) {
        // Manually update state for fast feedback
        final spotToRemove = spots.firstWhere((s) => s.id == spotId, orElse: () => spots.last);
        setState(() {
            spots.removeWhere((s) => s.id == spotId);
            parking = Parking(
                id: parking!.id, name: parking!.name, city: parking!.city, address: parking!.address, 
                totalSpots: parking!.totalSpots - 1, occupiedSpots: parking!.occupiedSpots - (spotToRemove.isOccupied ? 1 : 0), 
  ...
  void _calculateProjectedRevenue() {
    ...
    dailyEntries = (occupiedSpots * (24 / avgStayHours)).round();
  }

  // --- Utility Builder (Simplified Card) ---
  ...
  Widget build(BuildContext context) {
    ...
    final double occupancyPercentage = totalSpots > 0 ? (occupiedSpots / totalSpots) * 100 : 0.0;
    final config = _getCurrentTariffConfig();
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            ...
            icon: const Icon(Icons.monitor_heart_outlined, color: Colors.greenAccent),
            label: Text('Live Monitor', style: GoogleFonts.poppins(color: Colors.white)),
            ...
            Expanded(
              ...
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // TOP LEVEL TITLE (Non ripetuto)
                    Text('Tariff Management', style: GoogleFonts.poppins(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 16),
                    
                    // NEW: Unified Tariff Card
                    _buildCard(
                      title: 'Tariff & Rules',
                      child: TariffManagementCard(
                        ...
                        onStartTimeChanged: (newTime) => setState(() => _nightStartTime = newTime),
                        onEndTimeChanged: (newTime) => setState(() => _nightEndTime = newTime),
                      ),
                    ),
                      ...
                      child: SpotStatCard(
                        ...
                        onRemoveLastSpot: () {
                            if (lastSpot != null) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('No spots available to delete.')),
                                );
                            }
                        },
                      ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text('Save Tariff & Rules', style: GoogleFonts.poppins(color: Colors.black, fontWeight: FontWeight.bold)),
            ...
            Expanded(
              ...
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Cost Simulator', style: GoogleFonts.poppins(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 16),
                    
                    // Titles are now inside the Card widget for a more unified look
                    _buildCard(
            ...
            Expanded(
              ...
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Live Stats', style: GoogleFonts.poppins(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 16),
                    ...
                    _buildCard(
                      ...
                      child: DailyEntriesCard(dailyEntries: dailyEntries, avgStayHours: avgStayHours),
                    ),
                  ],
                ),
              ),
